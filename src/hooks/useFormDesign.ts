...
import { deserialize } from 'src/components/FormBuilder';

import { MetaFormInvoker } from '@/CodeDefine/Operation/Invoker/MetaFormInvoker';
import { KeyValueVO } from '@/CodeDefine/Operation/KeyValueVO';
...
import { ThirdApiFeatureTO } from '@/CodeDefine/Operation/ThirdApiFeatureTO';
import widgetsConfig from '@/pages/form/settings/widgetsConfig';
import {
  AliasTable,
  EntityRelation,
  setAliasTableList,
...
export default function useFormDesign() {
  ...
  const setFieldList = (fieldList: MetaFormFieldTO[]) => {
    dispatch(
      setFieldListAction({
        FieldList: fieldList,
      }),
  ...
  const setFormDetail = (detail: MetaFormRO) => {
    dispatch(
      setFormDetailAction({
        formDetail: detail,
      }),
  ...
  const persistDetail = (_formDetail: MetaFormRO) => {
